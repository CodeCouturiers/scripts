// ==UserScript==
// @name         Advanced Tmall Russian Translator
// @namespace    http://tampermonkey.net/
// @version      2.0
// @description  Расширенный русский перевод Tmall с поддержкой динамического контента
// @author       Your name
// @match        *://*.tmall.com/*
// @match        *://*.tmall.hk/*
// @grant        none
...
(function() {
    ...
    // Специфичные для Tmall переводы
    const tmallSpecificTranslations = {
        // Шапка сайта
        "天猫Tmall.com": "Tmall.com",
        "理想生活上天猫": "Идеальная жизнь на Tmall",
        "喵鲜生": "Свежие продукты",
        "天猫国际": "Tmall Global",
        "天猫超市": "Супермаркет Tmall",
        ...
        // Статусы заказов специфичные для Tmall
        "天猫配送": "Доставка Tmall",
        "天猫国际物流": "Международная доставка Tmall",
        "天猫直送": "Прямая доставка Tmall",

        // Промо-элементы
        "天猫商品": "Товары Tmall",
        "天猫商城": "Торговый центр Tmall",
        "品牌旗舰店": "Фирменный магазин",
        "官方旗舰店": "Официальный магазин",
        "专卖店": "Специализированный магазин",

        // Служба поддержки
        "天猫客服": "Служба поддержки Tmall",
        "在线客服": "Онлайн поддержка",
        "联系客服": "Связаться с поддержкой"
    };

    // Объединяем с общими переводами
    const translations = {
        ...tmallSpecificTranslations,
...
"我的积分": "Мои баллы",
    ...
    // Функция безопасного перевода текста
    ...
    function translateAttributes(element) {
        CONFIG.translateAttributes.forEach(attr => {
            if (element.hasAttribute(attr)) {
                ...
                const translatedText = safeTranslate(originalText);
    ...
    // Основная функция перевода элемента
    ...
    function init() {
        ...
        // Периодическая проверка новых элементов
        setInterval(() => {
            translateElement(document.body);
        }, CONFIG.detectInterval);

        if (CONFIG.debugMode) {
            console.log('Tmall translator initialized');
        }
    }
